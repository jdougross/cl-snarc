...
export const generateEmailText = (params: {
  ...
  const tourContactPosition = tourContact[TourContactFields.POSITION];
  const tourContactName = tourContact[TourContactFields.FULLNAME];
  const tourContactPhone = tourContact[TourContactFields.PHONE];
  const tourContactIndirectPronoun = tourContact[TourContactFields.PRONOUN];
  ...
  const acknowledgeLink = `Please click here to let us know you've gotten this message.`;
  ...
  const contactLine = `${tourContactPosition} ${tourContactName} is your contact. Please text/call ${tourContactIndirectPronoun} at ${tourContactPhone} upon arrival so that he can show you in and review things.  If there are any issues, please let us know with as much advance as possible at `;
